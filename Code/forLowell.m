            ...
            for ff = 1:length(fnames)
                ...
                if ~isempty(Fbeep) && ~isempty(Ffix) && num_single_units > 0
                    ...
                    for uu = 1:num_single_units
                        ...
                        Lg = isfinite(spike_rate_data(:,1,uu)) & isfinite(pupil_data(:,1)) & isfinite(pupil_data(:,2)) & zscore(spike_rate_data(:,2,uu))<10;
                        ...
                        spike_drift_slope(ith_unit) = spike_drift.Coefficients.Estimate(2);
                        pupil_drift = fitlm(all_trial_times,all_baseline_pd);
                        pupil_drift_slope(ith_unit) = pupil_drift.Coefficients.Estimate(2);
                        ...
                        LC_PD_data = cat(1, LC_PD_data, ...
                            ...
                            pupil_data(Lg,:) spike_drift.Residuals.Raw(1:sum(Lg)) sub_FR_evoked fix_start_times(Lg)]);
                        ...
                        lm_table = table(pupil_drift.Residuals.Raw(1:sum(Lg)), spike_drift.Residuals.Raw(1:sum(Lg)), sub_p_evoked, sub_FR_evoked,...
                        ...
                        % axis square;
                        % legend off;
                        [base_p_subevoked_p_R(ith_unit),base_p_subevoked_p(ith_unit)] = corr(pupil_drift.Residuals.Raw(1:sum(Lg)),sub_p_evoked,'type','Spearman');
                        ...
                        if base_p_subevoked_p(ith_unit) <0.05
                            if base_p_subevoked_p_R(ith_unit) > 0
                                pup_pup(ith_unit) = 2;  % positive relationship
                            else
                                pup_pup(ith_unit) = 3;  % negative relationship
                            end
                        else
                            pup_pup(ith_unit) = 1;      % no relationship
                        end
                        lme2 = fitlme(lm_table,'pupil_evoked ~ pupil_base*pupil_base');
                        results = compare(lme,lme2);
                        stats(ith_unit).pEvoked_v_pBase = results.pValue(2);
                        stats(ith_unit).pEvoked_v_pBase_LLR = results.LogLik(1)./results.LogLik(2);
                        ...
                        if sum(lm_table.spike_base)>0
                            ...
                            results = compare(lme,lme2);
                            stats(ith_unit).sEvoked_v_sBase = results.pValue(2);
                            stats(ith_unit).sEvoked_v_sBase_LLR = results.LogLik(1)./results.LogLik(2);
                        else
                            stats(ith_unit).sEvoked_v_sBase = NaN;
                            stats(ith_unit).sEvoked_v_sBase_LLR = NaN;
                        end
                        ...
                        h(3).Color = 'g'; 
                        h(4).Color = 'g';
                        [base_FR_subevoked_FR_R(ith_unit), base_FR_subevoked_FR(ith_unit)] = corr(spike_drift.Residuals.Raw(1:sum(Lg)), sub_FR_evoked,'type','Spearman');
                        % subplot(2,5,8);
                        ...
                        if sum(lm_table.spike_base)>0
                            ...
                            results = compare(lme,lme2);
                            stats(ith_unit).sEvoked_v_pBase = results.pValue(2);
                            stats(ith_unit).sEvoked_v_pBase_LLR = results.LogLik(1)./results.LogLik(2);
                        else
                            stats(ith_unit).sEvoked_v_pBase = NaN;
                            stats(ith_unit).sEvoked_v_pBase_LLR = NaN;
                        end
                        ...
                        h(3).Color = 'g'; 
                        h(4).Color = 'g';
                        [base_p_subevoked_FR_R(ith_unit), base_p_subevoked_FR(ith_unit)] = corr(pupil_drift.Residuals.Raw(1:sum(Lg)),sub_FR_evoked,'type','Spearman');
                        % subplot(2,5,6);
                        ...
                        %   b) baseline subtracted evoked
                        lm = fitlm(sub_p_evoked, spike_drift.Residuals.Raw(1:sum(Lg)),'linear');
                        lm2 = fitlm(sub_p_evoked, spike_drift.Residuals.Raw(1:sum(Lg)), 'purequadratic');
                        if sum(lm_table.spike_base)>0
                            lme = fitlme(lm_table,'pupil_evoked ~ spike_base');
                            lme2 = fitlme(lm_table,'pupil_evoked ~ spike_base*spike_base');
                            results = compare(lme,lme2);
                            stats(ith_unit).pEvoked_v_sBase = results.pValue(2);
                            stats(ith_unit).pEvoked_v_sBase_LLR = results.LogLik(1)./results.LogLik(2);
                        else
                            stats(ith_unit).pEvoked_v_sBase = NaN;
                            stats(ith_unit).pEvoked_v_sBase_LLR = NaN;
                        end
                        av = anova(lm2);
                        ...
                        h(3).Color = 'g'; 
                        h(4).Color = 'g';
                        [subevoked_p_base_FR_R(ith_unit), subevoked_p_base_FR(ith_unit)] = corr(sub_p_evoked, spike_drift.Residuals.Raw(1:sum(Lg)),'type','Spearman');
                        % subplot(2,5,10);
                        ...
                        spike_evoked_mag(ith_unit) = mean(sub_FR_evoked,'omitnan');
                        pupil_evoked_mag(ith_unit) = mean(sub_p_evoked,'omitnan');
                        ...
                        % if p(ith_unit) <0.05
                            ...
                            % if do_zscore
                            %     name = ['/Users/lowell/Library/Mobile Documents/com~apple~CloudDocs/UPenn Research/Code/LCPupil/Fits/Z_scored/',monkeys{mm},'/',extractBefore(fnames{ff},'.'),'_',num2str(ith_unit),'.pdf'];
                            % else
                            %     name = ['/Users/lowell/Library/Mobile Documents/com~apple~CloudDocs/UPenn Research/Code/LCPupil/Fits/',monkeys{mm},'/',pup_pup_folders{pup_pup(ith_unit)}, '/', extractBefore(fnames{ff},'.'),'_',num2str(ith_unit),'.pdf'];
                            % end
                            % saveas(h,name)
                        % end
                    end
...
figure; 
vars = {'monkey' 'session' 'unit', 'PD baseline' 'PD response' 'Spike baseline' 'Spike response'};
plots = [4 5; 6 7; 5 6];
...
for mm = 1:num_monkeys
    ...
    % Each plot
    for pp = 1:num_plots
        % For pupil only data we need to select sessions rather than all
        % units
        if pp == 1
            Lm = LC_PD_data(:,1)==mm & LC_PD_data(:,2);
        elseif pp == num_plots
            Lm = LC_PD_data(:,1)==mm & ismember(LC_PD_data(:,3), find(p <0.05));
            zs = LC_PD_data(Lm,end);
        else
            Lm = LC_PD_data(:,1)==mm; % & ismember(LC_PD_data(:,3), find(p <0.05));
        end
        ...
        if pp == num_plots
            if mm == 1
                plot3(xs, ys, zs, 'o','MarkerFaceColor',[0.5 0.5 0.5], 'MarkerEdgeColor','k');
            else
                plot3(xs, ys, zs,'d','MarkerFaceColor',[0.5 0.5 0.5], 'MarkerEdgeColor','k');
            end
        else
            if mm == 1
                plot(xs, ys, 'o','MarkerFaceColor',[0.5 0.5 0.5], 'MarkerEdgeColor','k');
            else
                plot(xs, ys, 'd','MarkerFaceColor',[0.5 0.5 0.5], 'MarkerEdgeColor','k');
            end
        end
        set(gca, 'FontSize', 12);
