...
plots = [4 6;... % baseline pupil, baseline FR
    4 5;... % baseline pupil, evoked pupil: Are evoked pupil responses inversely related to baseline pupil? Yes
    6 7;... % baseline FR, evoked FR: Are evoked spikes inversely related to baseline spikes?
    4 7;... % baseline pupil, evoked FR: Are evoked spikes inversely related to baseline pupil?
    5 6]; % evoked pupil, baseline FR: Are evoked pupil responses inversely related to tonic LC activity?
...
figure; 
vars = LC_Beep_labels; %{'monkey' 'session' 'unit', 'PD baseline' 'PD response' 'Spike baseline' 'Spike response'};
for mm = 1:num_monkeys
    ...
    % Each plot
    for pp = 1:num_plots
        subplot(num_plots,num_monkeys,sub2ind([num_monkeys,num_plots],mm,pp)); hold on;
        % if pp == 1
