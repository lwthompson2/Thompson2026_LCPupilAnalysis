function stats = unitSummaryPlotRaw(LC_Beep_table, LC_Fix_table)
%% Generates a unit summary plot for Joshi 2016 data and gets relevant statistics
% Created by LWT 9/24/2024
% Uses baseline activity that does not consider drift over time (i.e., not
% residuals)
...
[stats.base_p_base_FR, stats.p] = corr(all_baseline_FR',all_baseline_pd','Type','Spearman');
...
plot(all_baseline_pd,all_baseline_FR,'ok','MarkerFaceColor',[0.5 0.5 0.5])
j = lsline;
j.Color = 'k';
if stats.p <0.05
...
xlabel({'Baseline Pupil', '(raw)'})
ylabel({'Baseline FR', '(raw)'})
axis square;
...
[stats.base_p_subevoked_p_R, stats.base_p_subevoked_p] = corr(LC_Beep_table.pupil_baseline,LC_Beep_table.pupil_bs_evoked,'type','Spearman');

% Compare linear and quadratic models
% For plotting purposes we use fitlm. For stats we use fitlme
lm = fitlm(LC_Beep_table,'pupil_bs_evoked ~ pupil_baseline');
lm2 = fitlm(LC_Beep_table,'pupil_bs_evoked ~ pupil_baseline*pupil_baseline');
lme = fitlme(LC_Beep_table,'pupil_bs_evoked ~ pupil_baseline');
lme2 = fitlme(LC_Beep_table,'pupil_bs_evoked ~ pupil_baseline*pupil_baseline');
...
[stats.base_FR_subevoked_FR_R, stats.base_FR_subevoked_FR] = corr(LC_Beep_table.spike_baseline, LC_Beep_table.spike_bs_evoked,'type','Spearman');

% Use fitlm for plotting
lm = fitlm(LC_Beep_table, 'spike_bs_evoked ~ spike_baseline');
lm2 = fitlm(LC_Beep_table, 'spike_bs_evoked ~ spike_baseline*spike_baseline');
% To actually do this we need sufficient data
if numel(unique(LC_Beep_table.spike_baseline))>2
    % Use lme for model comparison
    lme = fitlme(LC_Beep_table, 'spike_bs_evoked ~ spike_baseline');
    lme2 = fitlme(LC_Beep_table, 'spike_bs_evoked ~ spike_baseline*spike_baseline');
    results = compare(lme,lme2);
...
[stats.base_p_subevoked_FR_R, stats.base_p_subevoked_FR] = corr(LC_Beep_table.pupil_baseline, LC_Beep_table.spike_bs_evoked,'type','Spearman');
...
% on both variables.
[stats.partial_base_p_evoked_FR_r, stats.partial_base_p_evoked_FR_p] = partialcorr(LC_Beep_table.pupil_baseline,... % baseline pupil
...
lm = fitlm(LC_Beep_table, 'spike_bs_evoked ~ pupil_baseline');
lm2 = fitlm(LC_Beep_table, 'spike_bs_evoked ~ pupil_baseline*pupil_baseline');

if numel(unique(LC_Beep_table.spike_baseline))>2
    lme = fitlme(LC_Beep_table, 'spike_bs_evoked ~ pupil_baseline');
    lme2 = fitlme(LC_Beep_table, 'spike_bs_evoked ~ pupil_baseline*pupil_baseline');
    results = compare(lme,lme2);
...
[stats.subevoked_p_base_FR_R, stats.subevoked_p_base_FR] = corr(LC_Beep_table.pupil_bs_evoked, LC_Beep_table.spike_baseline,'type','Spearman');
[stats.partial_base_FR_evoked_p_r, stats.partial_base_FR_evoked_p_p] = partialcorr(LC_Beep_table.spike_baseline,...
    LC_Beep_table.pupil_bs_evoked+LC_Beep_table.pupil_baseline,...
    LC_Beep_table.pupil_baseline,'Type','Spearman');

lm = fitlm(LC_Beep_table,'pupil_bs_evoked ~ spike_baseline');
lm2 = fitlm(LC_Beep_table,'pupil_bs_evoked ~ spike_baseline*spike_baseline');

if numel(unique(LC_Beep_table.spike_baseline))>2
    lme = fitlme(LC_Beep_table,'pupil_bs_evoked ~ spike_baseline');
    lme2 = fitlme(LC_Beep_table,'pupil_bs_evoked ~ spike_baseline*spike_baseline');
    results = compare(lme,lme2);
