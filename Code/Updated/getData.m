            ...
            for ff = 1:length(fnames)
                ...
                if ~isempty(Fbeep) && ~isempty(Ffix) && num_single_units > 0
                    ...
                    for uu = 1:num_single_units
                        ...
                        stats(ith_unit) = unitSummaryPlot(LC_Beep_data(LC_Beep_data(:,3)==ith_unit,:), LC_Fix_data(LC_Fix_data(:,3)==ith_unit,:), ith_unit);
...
for mm = 1:num_monkeys
    ...
    for pp = 1:num_plots
        ...
        % units
        if pp == 1
            Lm = LC_Beep_data(:,1)==mm & LC_Beep_data(:,2);
        elseif pp == num_plots
            Lm = LC_Beep_data(:,1)==mm & ismember(LC_Beep_data(:,3), find(stats.p <0.05));
            zs = LC_Beep_data(Lm,end);
        else
            Lm = LC_Beep_data(:,1)==mm; % & ismember(LC_PD_data(:,3), find(p <0.05));
        end
